...
on *:load:{ 
  ...
  echo -a  4Team Nexgen's Secure Query v2.17 | echo -a  4Ju lutem shtypni: /secure.query ne kanal qe tju dali dialogu.
}
...
menu query,menubar {
  Bisedim Ne Privat
  .config:secure.query
  .add access:.auser -a securequery $address($1,3) | .msg $1 [auto mesazh] ju jeni ne Aces ne msg privat me mua!D. | $ae •12• user added » (12 $address($1,3) ) | if ($dialog(querycon) != $null) { did -r querycon 45 | s.levels } 
...
dialog querycon1 {
  title "BEZDISJA NE PRIVAT"
  size -1 -1 160 81
  option dbu
  box "", 33, 7 14 146 42
  button "Pranoje", 28, 9 62 25 10, disable default ok
  button "Injoroje", 26, 36 62 25 10, disable
  button "Shperfille", 27, 63 62 25 10, disable
  ...
  button "Menuja", 2, 92 63 21 8
  button "Shtoje ne liste", 3, 114 63 37 8, disable
  box "", 4, 7 57 146 18
...
dialog querycon {
  ...
  link "www.RiniaLive.eu", 59, 56 80 59 8, tab 54
  text "K|nG", 60, 84 31 22 8, tab 54
  text "v2.17", 61, 84 42 14 8, tab 54
  text "sigurimi i privatit eshte bere nga K|nG ne versionin me te ri.", 63, 34 55 97 20, tab 54
}
on *:dialog:*:init:*:{ 
...
on *:dialog:*:sclick:*:{ 
  if (Query request $dname == $dialog($dname).title) { 
    ...
    if ($did == 3) {
      ...
      if ($_vr(querycon,notifyaccess) == on) { if ($server != $null) { .msg $did(1,$did(1)) [auto mesazh] You have free access to message me. } }
    }   
    if ($did == 27) { .ignore -pu180 $did(30,$did(30)) | .msg $did(1,$did(1)) [Auto mesazh] $_vr(querycon,decline) |  dialog -x $did(1,$did(1)) }
    if ($did == 28) {  
      ...
      if ($server != $null) { .msg $did(1,$did(1)) 4,1[auto mesazh] mesazhi eshte pranuar! . }
...
on ^*:open:?: {
  ...
  if ($_vr(querycon,netflood) != on) {
    ...
    if (%upflood15 > $_vr(querycon,max.query)) { 
      ...
      echo -a 4[ KUJDES ] $nick  eshte kapur me flood ne privat. Useri ( $+ $wildsite $+ ) eshte futur ne ignore list  per $_vr(querycon,max.query.sec) secs.
    } 
  ...
  if ($_vr(querycon,switch) == on) {
    ...
    .msg $nick [auto mesazh] $_vr(querycon,standby)
